...
class Conv2d(torch.nn.Module):
    def __init__(self,
        ...
        resample_filter=[1,1], fused_resample=True, init_mode='kaiming_normal', init_weight=1, init_bias=0,
    ):
    ...
    def forward(self, x):
        ...
        if self.fused_resample and self.up and w is not None:
            #x = torch.nn.functional.conv_transpose2d(x, tw_, groups=self.in_channels, stride=2, padding=max(f_pad - w_pad, 0))
            x = torch.nn.functional.interpolate(x, scale_factor=2, mode='nearest')
            #x = torch.nn.functional.conv_transpose2d(x, f.mul(4).tile([self.in_channels, 1, 1, 1]), groups=self.in_channels, stride=2, padding=max(f_pad - w_pad, 0))
            x = torch.nn.functional.conv2d(x, w, padding=max(w_pad - f_pad, 0))
            #x = torch.nn.functional.conv2d(x, w_, padding=max(w_pad - f_pad, 0))
            #x = silu(x)
            #x = self.shuffle(x)
...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        self.conv1 = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=3, **init)
        ...
        if self.num_heads:
            ...
            self.proj = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=1, **init)
    ...
    def __init__(self,
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f'{resx}x{resy}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True, attention=False, **block_kwargs)
            for idx in range(num_blocks):
                cin = cout
                cout = model_channels * mult
                self.enc[f'{resx}x{resy}_block{idx}'] = UNetBlock(
                    in_channels=cin, out_channels=cout, attention=False, **block_kwargs) # or = 
        ...
        self.out_conv = Conv2d(in_channels=cout, out_channels=out_channels, kernel=3, **init)
