...
if __name__ == "__main__":
    ...
    torch.save(det_model.state_dict(), f"src/checkpoints/deterministic_unet_dom128128.pt")
    ...
    np.save(f"src/codebase/predictions/deterministic_unet_dom128128.npy", preds.numpy())
    ...
    for e in range(1, epoch - args.patience + 1):
        if e <= args.warmup_epochs:
            ...
            prob_model_es.beta_1 = 5*betas[e-args.warmup_epochs-1]
        else:
            prob_model_es.beta_1 = 5
        tr_mae, tr_kl = tu.train_step(model=prob_model, dataloader=megatrainloader, loss_fn=torch.nn.L1Loss(), optimizer=optimizer_prob, prob=True, epoch=e, device=args.device)
    ...
    np.save(f"src/logs/probabilistic_unet_beta_mae.npy", np.array(training_mae))
    np.save(f"src/logs/probabilistic_unet_beta_kl.npy", np.array(training_kl))
    torch.save(prob_model.state_dict(), f"src/checkpoints/probabilistic_unet_beta.pt")
    ...
    np.save(f"src/codebase/predictions/probabilistic_unet_beta.npy", preds.numpy())
